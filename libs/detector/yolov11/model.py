...
from libs.detector.yolov11.preprocess import preProcessPadding
from libs.detector.yolov11.postprocess.postprocess import PostProcessor_YOLOV11Pose
from libs.detector.yolov11.postprocess.postprocess import IMAGE_SIZE_YOLOV11, THRESHOLD_YOLOV11
import cv2

CURRENT_DIR = os.path.abspath(os.path.dirname(__file__)).split('libs')[0]


class YOLOv11Pose(object):
    def __init__(self, file='./config/human/yolov11s-pose.onnx', class_sel=[]):
    ...
    def forward(self, image):
        ...
        results_batch = PostProcessor_YOLOV11Pose(out, len(self.classes))
        ...
        for result in results_batch:
            if len(result) > 0:
                ...
                result = [r for r in result if r[4] > THRESHOLD_YOLOV11]
                for r in result:
                    x, y, x2, y2, score, label = r
                    if int(label) > len(self.classes) - 1 or self.classes[int(label)] not in self.class_sel:
                        continue

                    w = IMAGE_SIZE_YOLOV11 / max(oriX, oriY)

                    y = (y - (IMAGE_SIZE_YOLOV11 - w * oriY) / 2) / w + 1
                    y2 = (y2 - (IMAGE_SIZE_YOLOV11 - w * oriY) / 2) / w + 1
                    x = (x - (IMAGE_SIZE_YOLOV11 - w * oriX) / 2) / w + 1
                    x2 = (x2 - (IMAGE_SIZE_YOLOV11 - w * oriX) / 2) / w + 1
